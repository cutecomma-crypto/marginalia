...
export const MOTIVATION_TAGS = ['好奇', '解決問題', '工作需要', '自我成長', '學習某個主題', '別人推薦', '隨意閱讀', '其他'];
...
export async function renderMotivation(container, bookId) {
  ...
  container.innerHTML = `
    ...
    <form id="motivation-form" class="book-form">
      <label>可以選擇（可複選）
        <span class="tag-checkboxes">${tagCheckboxes('motivationTags', MOTIVATION_TAGS, existing && existing.tags)}</span>
      </label>
      <label>我為什麼想看這本書？
...
export async function renderReflections(container, bookId) {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    await DB.add('outputs', { bookId, kind: 'reflection', tags, text });
